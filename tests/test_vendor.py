...
class VendorTest(unittest.TestCase):
    ...
    def test_sync_verify_and_detect_tampering(self) -> None:
        ...
        with patch("builtins.print") as output:
            VENDOR.sync(ROOT, self.target, "v0.1.1", commit)
    ...
    def test_release_identity_requires_clean_exact_tag(self) -> None:
        with patch.object(VENDOR, "git_output", side_effect=["", "b" * 40, "v0.1.1"]):
            self.assertEqual("b" * 40, VENDOR.release_identity(ROOT, "v0.1.1"))
        with (
            patch.object(VENDOR, "git_output", return_value="dirty"),
            self.assertRaisesRegex(RuntimeError, "must be clean"),
        ):
            VENDOR.release_identity(ROOT, "v0.1.1")
        with (
    ...
    def test_verify_rejects_every_identity_and_manifest_boundary(self) -> None:
        ...
        core.write_text(
            core.read_text().replace(
                'COORDINATOR_VERSION = "0.1.1"', 'COORDINATOR_VERSION = "9.9.9"'
            )
        )
        ...
        with (
            patch.object(VENDOR, "git_output", side_effect=["", "short", "v0.1.1"]),
    ...
    def test_main_dispatches_sync_and_verify(self) -> None:
        ...
        with (
            ...
            self.assertEqual(0, VENDOR.main())
            sync.assert_called_once_with(ROOT, self.target, "v0.1.1", "c" * 40)
