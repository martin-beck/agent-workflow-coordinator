...
class AuthorityEffectCorrespondenceTests(unittest.TestCase):
    ...
    def test_model_contract_rejects_weakened_barrier_safety_invariant(self) -> None:
        model = ROOT / "formal/upgrade/HandoffctlUpgradeBarrier.tla"
        original = model.read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as directory:
            temporary_root = Path(directory)
            ...
            temporary_model.write_text(
                original.replace(
                    ...
                    'sessionStatus = "released" => TRUE',
                    1,
                ),
                encoding="utf-8",
            )
            with self.assertRaisesRegex(ValueError, "ReleaseEvidence semantics"):
                validate_authority_effect_model_contract(temporary_root)

    def test_model_contract_rejects_weakened_session_identity_invariant(self) -> None:
        model = ROOT / "formal/upgrade/HandoffctlUpgradeBarrier.tla"
        original = model.read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as directory:
            temporary_root = Path(directory)
            ...
            temporary_model.parent.mkdir(parents=True)
            temporary_model.write_text(
                original.replace("fence > 0", "fence >= 0", 1),
                encoding="utf-8",
            )
            with self.assertRaisesRegex(ValueError, "IdentityStable semantics"):
                validate_authority_effect_model_contract(temporary_root)

    def test_model_contract_rejects_weakened_type_invariant(self) -> None:
        model = ROOT / "formal/upgrade/HandoffctlUpgradeBarrier.tla"
        original = model.read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as directory:
            temporary_root = Path(directory)
            ...
            temporary_model.parent.mkdir(parents=True)
            temporary_model.write_text(
                original.replace("sessionStatus \\in Statuses", "sessionStatus \\in {}", 1),
                encoding="utf-8",
            )
            with self.assertRaisesRegex(ValueError, "TypeOK field sessionStatus"):
                validate_authority_effect_model_contract(temporary_root)
    ...
    def test_model_contract_requires_terminal_transition_semantics(self) -> None:
        model = ROOT / "formal/upgrade/HandoffctlUpgradeBarrier.tla"
        original = model.read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as directory:
            temporary_root = Path(directory)
            ...
            temporary_model.parent.mkdir(parents=True)
            temporary_model.write_text(
                original.replace("rollbackChild' = RollbackId(p)", "rollbackChild' = NoChild", 1),
                encoding="utf-8",
            )
            with self.assertRaisesRegex(ValueError, "BindRollback transition"):
                validate_authority_effect_model_contract(temporary_root)
